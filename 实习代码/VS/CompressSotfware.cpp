 ...
 * @LastEditors: DLCT
 * @LastEditTime: 2022-07-18 10:15:37
 */
#include <iostream>
#include <fstream>
#include <string>
...
void changeFileName(char* filename, char* newextension); //改变文件扩展名
...
double compressFile(char* filename)
{
    ...
    changeFileName(filename, hzm);      //修改文件后缀，并获得文件原始后缀名
    ...
    output.close();
    //返回压缩率
    cout << "压缩后文件大小为：" << (1.0 * count_ac) / 1048576 << "MB" << endl;
